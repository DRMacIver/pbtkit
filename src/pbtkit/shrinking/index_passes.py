...
def _indexed_indices(shrinker: Shrinker) -> list[int]:
    """Return indices of all nodes whose kind supports to_index/from_index."""
    return [i for i, n in enumerate(shrinker.current.nodes) if n.kind.supports_index]


@shrink_pass
def lower_and_bump(shrinker: Shrinker) -> None:
    ...
    for gap in range(1, min(len(_indexed_indices(shrinker)), 4)):
        idx = 0
        while idx < len(_indexed_indices(shrinker)):
            indices = _indexed_indices(shrinker)
            i = indices[idx]
            node_i = shrinker.current.nodes[i]
            kind_i = node_i.kind
            ...
            for new_val in decrement_targets:
                ...
                if tc.status is not None and j < len(tc.nodes):
                    zeroed2_values = [n.value for n in shrinker.current.nodes]
                ...
                def _try_bump_j(val: object, _new_val: object = new_val) -> bool:
                    ...
                    return (
                        j < len(nodes)
                        and nodes[j].kind.validate(val)
                        and shrinker.replace({i: _new_val, j: val})
                    )

                if (
                    j < len(shrinker.current.nodes)
                    and shrinker.current.nodes[j].kind.supports_index
                ):
                    kind_j = shrinker.current.nodes[j].kind
                    target_idx = kind_j.to_index(shrinker.current.nodes[j].value)
                    for bump in [1, 2, 4]:
...
@shrink_pass
def try_shortening_via_increment(shrinker: Shrinker) -> None:
    ...
    while i < len(shrinker.current.nodes):
        ...
        for incremented in candidates:
            ...
            shrinker.test_function(tc_zeroed)
        i += 1
